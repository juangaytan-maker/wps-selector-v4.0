/* ============================================================================
   🔑 PRO SYSTEM & LICENSE VALIDATION - v4.2
   ============================================================================ */
...
export function updateProUI() {
    ...
    if (isProUser) {
        ...
        if (proActiveDiv) proActiveDiv.style.display = 'block';        if (activationCard) activationCard.classList.add('pro-active');
...
export async function activatePro(licenseCode) {
    ...
    try {
        const docRef = doc(db, 'licenses', cleanCode);        const docSnap = await getDoc(docRef);
        ...
        if (data.status !== 'active') {
            return { success: false, message: '❌ Licencia inactiva o caducada.' };
        }
...
export function closeLogoutModal() {    const modal = document.getElementById('logoutModal');
